...
- **CSV_EXPORT_SCHEDULE**: CSV送信スケジュール（daily/weekly/monthly/cron形式）
- **CSV_EXPORT_DAYS**: CSV送信日数（デフォルト: 30）
- **SMTP_HOST**: SMTPサーバーホスト名
- **SMTP_PORT**: SMTPポート番号
- **SMTP_USER**: SMTP認証ユーザー名
- **SMTP_PASSWORD**: SMTP認証パスワード
- **SMTP_USE_TLS**: SMTP TLS使用フラグ（true/false）
