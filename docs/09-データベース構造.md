...
- **User削除時**: 関連するShiftとBreakは `cascade="all, delete-orphan"` で削除され、AuditLogは `user_id` が `NULL` に更新される
...
アプリ起動時に `init_database()` が呼ばれ、必要なテーブルを作成します:
